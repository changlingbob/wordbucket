    ...
    describe('command words', () => {
        ...
        describe('set', () => {
            ...
            it('throws on misformed commands', () => {
                expect(() => new Word('${$set}').generate({})).toThrow(SetVariableError);
                expect(() => new Word('${$set foo}').generate({})).toThrow(SetVariableError);
                expect(() => new Word('${$set foo bar}').generate({})).not.toThrow(SetVariableError);
                expect(() => new Word('${$set foo bar baz}').generate({})).toThrow(SetVariableError);
            });
            it('stores data', () => {
                const output = {};
        ...
        describe('var', () => {
            it('throws on misformed commands', () => {
                expect(() => new Word('${$var}').generate({})).toThrow(ReadVariableError);
                expect(() => new Word('${$var foo}').generate({ foo: 'test' })).not.toThrow(ReadVariableError);
                expect(() => new Word('${$var foo bar}').generate({ foo: 'foo' })).not.toThrow(ReadVariableError);
            });
            it('loads data', () => {
        ...
        describe('table', () => {
            ...
            it("throws if a variable doesn't specify a table", () => {
                expect(() => new Word('${$table test}').generate({ test: 'non-existant' })).toThrow(MissingBucketError);
            });
        });
    });
});
